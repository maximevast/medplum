...
function extractAvailabilityR4(ext: {
  ...
  return ext.extension
    .filter((sub): sub is AvailabilityR4AvailableTime => sub.url === 'availableTime')
    .map((availTime) => {
      const dayOfWeek = availTime.extension
        .filter((e): e is { url: 'daysOfWeek'; valueCode: DayOfWeek } => e.url === 'daysOfWeek')
        .map((e) => e.valueCode);
      const allDay = availTime.extension.find(
        (e): e is { url: 'allDay'; valueBoolean: boolean } => e.url === 'allDay'
      )?.valueBoolean;
      const start = availTime.extension.find(
        (e): e is { url: 'availableStartTime'; valueTime: `${number}:${number}:${number}` } =>
          e.url === 'availableStartTime'
      )?.valueTime;
      const end = availTime.extension.find(
        (e): e is { url: 'availableEndTime'; valueTime: `${number}:${number}:${number}` } =>
          e.url === 'availableEndTime'
      )?.valueTime;
...
export function parseSchedulingParametersExtensions(resource: Schedule | HealthcareService): SchedulingParameters[] {
  ...
  return extensions.map((extension) => {
    ...
    const rawAvailabilityR4 = extension.extension.filter(
      (
        ext
      ): ext is {
        url: 'availability.r4';
        extension: (AvailabilityR4AvailableTime | AvailabilityR4NotAvailableTime)[];
      } => ext.url === 'availability.r4'
    );
